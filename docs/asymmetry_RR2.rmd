...
```{r trialstructure, echo=FALSE, fig.cap= "Experiment design. Metacognitive asymmetry effects were tested for six stimulus features in six separate experiments, ecompassing three levels of abstraction: local features, global features, and expectation violations. The presented trial corresponds to the first stimulus pair, were *Q* and *O* are used as stimuli.", out.width = '100%'}
...
We aimed to follow our pre-specified methods as close as possible (the approved Stage 1 protocol is available in the following link: doi.org/10.31234/osf.io/av9ns). The following is an exhaustive list of deviations from our pre-registered plan.

- *Stimulus used for backward masking*: We planned to use the same stimulus (the letter *Z*) for backward masking in all six experiments. This mask was effective in Experiments 1 and 2, but in Experiment 3 overly high accuracy levels indicated that for these stimuli the mask was not salient enough.  For a subset of participants in Exp. 3, an overlay of all 10 stimuli from all 6 experiments (*Q*, *O*, *C*, vertical, tilted, and curved lines, upward-tilted and downward-tilted cubes, and normal and flipped Ns) was used. For the remaining participants and experiments, we used four dollar signs as our mask.
...
- *Monetary compensation*: For some of the experiments, we noticed that that participants completed the experiment more quickly than what we had originally estimated. We therefore reduced our offered payment for some of the experiments, while maintaining a median hourly wage of £7 or higher.

# Results

A summary of the results from all six experiments is available in section \@ref(summary) and in Figures \@ref(fig:all-distributions), \@ref(fig:all-rcROCs) and \@ref(fig:summary).
...
```{r load_e1, echo=FALSE, cache=TRUE}
...
Median completion time was `r printnum(this.expt$median_completion_time)` minutes. Mean accuracy was `r printnum(this.expt$disc_df_unfiltered$correct%>%mean())`. Participants reported seeing an *O* on `r printnum(this.expt$disc_df_unfiltered$response%>%as.numeric()%>%mean()-1)` of the trials. Overall we excluded `r this.expt$N_total-this.expt$N` participants based on our exclusion criteria, leaving `r this.expt$N` participants for the main analysis. Due to a technical error in data collection, this figure is higher than that specified in our preregistration document (N=106). Going forward, only data from included participants is analyzed.

```{r analyze_e1, echo=FALSE, cache=TRUE}
...
*Hypothesis 1*: In line with our hypothesis, confidence was generally higher for *Q* (feature present) responses than for *O* (feature absent) responses (`r apa_print(this.expt$H1$t.test)$statistic`; Cohen's d = `r printnum(this.expt$H1$d)`; `r apa_print(this.expt$H1$BF)$statistic`; see Fig. \@ref(fig:all-distributions), panel 1).

*Hypothesis 2*: In order to measure metacognitive asymmetry, we extracted the response-conditional type-2 ROC (rc-ROC) curves for the two responses (*Q* and *O*) in the discrimination task. This was done by plotting the cumulative distribution of confidence ratings (high to low) for correct responses against the same distribution for incorrect responses. The area under the rc-ROC curve (auROC) was then taken as a measure of metacognitive sensitivity [@kanai2010subjective; @meuwese2014subjective]. In line with our hypothesis, auROC for *Q* responses (`r apa_print(t.test(this.expt$AUC$response1))$estimate`) was higher than for *O* responses (`r apa_print(t.test(this.expt$AUC$response2))$estimate`; `r apa_print(this.expt$H2$t.test)$statistic`; Cohen's d = `r printnum(this.expt$H2$d)`; `r apa_print(this.expt$H2$BF)$statistic`; see Figure \@ref(fig:all-rcROCs), panel 1), similar to the documented metacognitive asymmetry for detection judgments.
...
*Hypothesis 4*: In line with our hypothesis, *Q* responses were faster on average than *O* responses by `r e1$H4$raw_es%>%abs()%>%round()` ms. (`r apa_print(this.expt$H4$t.test)$statistic` ; Cohen's d = `r printnum(this.expt$H4$d)`; `r apa_print(this.expt$H4$BF)$statistic`; see Fig. \@ref(fig:all-distributions), panel 1).
...
e1 <- plotROC(e1, c('Q','O'), 'figures/exp1.png')

```
```{r all-distributions, echo=FALSE, fig.cap= "Reaction time and confidence distributions for Experiments 1-6. Box edges and central lines represent the 25, 50 and 75 quantiles. Whiskers cover data points within four inter-quartile ranges around the median. Black lines connect the median values for the two responses. Stars reprsent significance in a two-sided t-test: **: p<0.01, ***: p<0.001", out.width = '100%'}
knitr::include_graphics("figures/hists.png")
```

```{r all-rcROCs, echo=FALSE, fig.cap= "Response conditional ROC curves for Experiments 1-6. The area under the curve is a measure of metacognitive sensitivity. Error bars stand for the standard error of the mean. For illustration, the response-conditional ROC curves of the first 20 participants of each Experiment are plotted in low opacity. Below each ROC: distributions of the area under the curve for the two responses, across participants. Each line corresponds to a participant. Stars reprsent significance in a two-sided t-test: *: p<0.05, **: p<0.01, ***: p<0.001", out.width = '100%'}
knitr::include_graphics("figures/rcROCs.png")
```



## Experiment 2: C vs. O
In Experiment 2, we examined discrimination judgments between the two letters *C* and *O*. Based on a search asymmetry for these letters [*C*s are found faster among *O*s than vice versa; @treisman1985search; @takeda2000inhibitory; @treisman1988feature], we hypothesized that a similar asymmetry would emerge in subjective confidence judgments, such that metacognitive sensitivity for perceiving a *C* will be higher than for perceiving an *O*. We used the letter *Z* as our backward mask. 

```{r e2, echo=FALSE, fig.show='hide', cache=TRUE}
...
e2 <- e2%>%
  ...
  testH3() %>%
  testH4() %>%
  plotROC(c('C','O'),'figures/exp2.png');
...
Median completion time was `r printnum(this.expt$median_completion_time)` minutes. Mean accuracy was `r printnum(this.expt$disc_df_unfiltered$correct%>%mean())`, and participants reported seeing an *O* on `r printnum(this.expt$disc_df_unfiltered$response%>%as.numeric()%>%mean()-1)` of the trials. Overall we excluded `r this.expt$N_total-this.expt$N` participants, leaving `r this.expt$N` participants for the main analysis. Going forward, only data from included participants is analyzed.
...
*Hypothesis 1*: In line with our hypothesis, confidence was generally higher for *C* (feature present) responses than for *O* (feature absent) responses (`r apa_print(this.expt$H1$t.test)$full_result`; Cohen's d = `r printnum(this.expt$H1$d)`; `r apa_print(this.expt$H1$BF)$statistic`; see Figure \@ref(fig:all-rcROCs), panel 2).

*Hypothesis 2*: Opposite to our prediction, auROC for *C* responses (`r apa_print(t.test(this.expt$AUC$response1))$estimate`) was *lower* than for *O* responses (`r apa_print(t.test(this.expt$AUC$response2))$estimate`; `r apa_print(this.expt$H2$t.test)$statistic`; Cohen's d = `r printnum(this.expt$H2$d)`; see Figure \@ref(fig:all-rcROCs), panel 2.). Bayes Factor strongly supported the alternative (`r apa_print(this.expt$H2$BF)$statistic`). Note that our prior on effect sizes was symmetric around zero, such that support for the alternative is obtained for negative, as well as positive effects. 
...
*Hypothesis 4*: Contrary to our hypothesis, response times for *C* and for *O* responses were highly similar, with a median difference of `r e2$H4$raw_es%>%abs()%>%round()` ms. (`r apa_print(this.expt$H4$t.test)$statistic` ; Cohen's d = `r printnum(this.expt$H4$d)`; `r apa_print(this.expt$H4$BF)$statistic`; see Figure \@ref(fig:all-rcROCs), panel 2).
...
In Experiment 3, we examind discrimination judgments between tilted and vertical lines. Based on a search asymmetry for these stimuli [tilted lines are found faster among vertical lines than vice versa; @treisman1988feature], we hypothesized that a similar asymmetry would emerge in subjective confidence judgments, such that metacognitive sensitivity for perceiving a tilted line will be higher than for perceiving a vertical line. As described in section \@ref(deviations), overly high accuracy in the first few participants led us to change our masking stimulus, first to an overlay of all stimuli and then to four dollar signs. We present here the combined results from these last two cohorts of participants. The results were qualitatively similar in the two cohorts. 

```{r e3, echo=FALSE, fig.show='hide', cache=TRUE}

e3=list()

e3$df <- read_csv('../experiments/Experiment3/data/jatos_results_batch1.csv') %>%
  rename('subj_id' = 'subject_identifier')

e3$export <- read_csv('../experiments/Experiment3/data/prolific_export_batch1.csv')  %>%
  rename('subj_id' = 'participant_id')
...
e3 <- e3%>%
  ...
  testH3() %>%
  testH4() %>%
  plotROC(c('tilted','vertical'),'figures/exp3.png')
...
`r this.expt$N_total` participants were recruited from Prolific for Experiment 3. 

Median completion time was `r printnum(this.expt$median_completion_time)` minutes. Mean accuracy was `r printnum(this.expt$disc_df_unfiltered$correct%>%mean())`, and participants reported seeing a vertical line on `r printnum(this.expt$disc_df_unfiltered$response%>%as.numeric()%>%mean()-1)` of the trials. Overall we excluded `r this.expt$N_total-this.expt$N` participants, leaving `r this.expt$N` participants for the main analysis. Going forward, only data from included participants is analyzed. 

Mean accuracy among included participants was `r apa_print(this.expt$disc_subj_stats$acc%>%t.test())$estimate`. The mean SOA of the last trial was `r apa_print(t.test(this.expt$final_trial$measured_SOA))$estimate`. Participants showed a consistent bias toward reporting a tilted rather than a vertical line (`r apa_print(this.expt$disc_subj_stats$bias%>%t.test())$estimate`). On a scale of 0 to 1, mean confidence level was `r apa_print(this.expt$disc_subj_stats$conf%>%t.test())$estimate`. Confidence was higher for correct than for incorrect responses (`r apa_print(this.expt$conf_by_acc)$full_result`). 

*Hypothesis 1*: In line with our hypothesis, confidence was generally higher for tilted lines (feature present) responses than for vertical lines (feature absent) responses (`r apa_print(this.expt$H1$t.test)$full_result`; Cohen's d = `r printnum(this.expt$H1$d)`; `r apa_print(this.expt$H1$BF)$statistic`; see Figure \@ref(fig:all-rcROCs), panel 3).

*Hypothesis 2*: Contrary to our prediction, auROC for reports of seeing a tilted line (`r apa_print(t.test(this.expt$AUC$response1))$estimate`) was similar to auROC for reports of seeing a vertical line according to Bayes Factor analysis (`r apa_print(t.test(this.expt$AUC$response2))$estimate`; Cohen's d = `r printnum(this.expt$H2$d)`; `r apa_print(this.expt$H2$BF)$statistic`; see Figure \@ref(fig:all-rcROCs), panel 3.). This difference in metacognitive sensitivity was however significant in a standard t-test (`r apa_print(this.expt$H2$t.test)$statistic`).
...
*Hypothesis 4*: In line with our hypothesis, response times for 'tilted' responses were significantly faster than response times for 'vertical' responses, with a median difference of `r e3$H4$raw_es%>%abs()%>%round()` ms. (`r apa_print(this.expt$H4$t.test)$statistic` ; Cohen's d = `r printnum(this.expt$H4$d)`; `r apa_print(this.expt$H4$BF)$statistic`; see Figure \@ref(fig:all-rcROCs), panel 3).

In summary, in Experiment 3 we found that 'tilted' (feature present) responses were faster and accompanied by higher subjective confidence that 'vertical' (feature absent) responses, in the absence of a difference in metacognitive sensitivity between the two responses. 

## Experiment 4: curved vs. straight lines

In Experiment 4, we examined discrimination judgments between curved and vertical lines. Based on a search asymmetry for these stimuli [curved lines are found faster among vertical lines than vice versa; @treisman1988feature], we hypothesized that a similar asymmetry would emerge in subjective confidence judgments, such that metacognitive sensitivity for perceiving a tilted line will be higher than for perceiving a vertical line. We used four dollar signs (\$\$\$\$) as our mask.

```{r e4, echo=FALSE, fig.show='hide', cache=TRUE}

e4=list()

e4$df <- read_csv('../experiments/Experiment4/data/jatos_results_batch1.csv') %>%
  rename('subj_id' = 'subject_identifier')

e4$export <- read_csv('../experiments/Experiment4/data/prolific_export_batch1.csv')  %>%
  rename('subj_id' = 'participant_id')
...
e4 <- e4%>%
  ...
  testH3() %>%
  testH4() %>%
  plotROC(c('curved','straight'),'figures/exp4.png')

this.expt <- e4;

```

`r this.expt$N_total` participants were recruited from Prolific for Experiment 4. 

Median completion time was `r printnum(this.expt$median_completion_time)` minutes. Mean accuracy was `r printnum(this.expt$disc_df_unfiltered$correct%>%mean())`, and participants reported seeing a straight line on `r printnum(this.expt$disc_df_unfiltered$response%>%as.numeric()%>%mean()-1)` of the trials. Overall we excluded `r this.expt$N_total-this.expt$N` participants, leaving `r this.expt$N` participants for the main analysis. Going forward, only data from included participants is analyzed. 

Mean accuracy among included participants was `r apa_print(this.expt$disc_subj_stats$acc%>%t.test())$estimate`. The mean SOA of the last trial was `r apa_print(t.test(this.expt$final_trial$measured_SOA))$estimate`. Participants showed a consistent bias toward reporting a curved rather than a vertical line (`r apa_print(this.expt$disc_subj_stats$bias%>%t.test())$estimate`). On a scale of 0 to 1, mean confidence level was `r apa_print(this.expt$disc_subj_stats$conf%>%t.test())$estimate`. Confidence was higher for correct than for incorrect responses (`r apa_print(this.expt$conf_by_acc)$full_result`). 

*Hypothesis 1*: In line with our hypothesis, confidence was generally higher for curved lines (feature present) responses than for straight lines (feature absent) responses (`r apa_print(this.expt$H1$t.test)$full_result`; Cohen's d = `r printnum(this.expt$H1$d)`; `r apa_print(this.expt$H1$BF)$statistic`; see Figure \@ref(fig:all-rcROCs), panel 4).

*Hypothesis 2*: Contrary to our prediction, auROC for reports of seeing a curved line (`r apa_print(t.test(this.expt$AUC$response1))$estimate`) was similar to auROC for reports of seeing a straight line (`r apa_print(t.test(this.expt$AUC$response2))$estimate`; `r apa_print(this.expt$H2$t.test)$statistic`; Cohen's d = `r printnum(this.expt$H2$d)`; `r apa_print(this.expt$H2$BF)$statistic`; see Figure \@ref(fig:all-rcROCs), panel 4.). 

*Hypothesis 3*: (The lack of) metacognitive asymmetry was not different from what would be expected based on an equal-variance SDT model with the same response bias and sensitivity (`r apa_print(this.expt$H3$t.test)$statistic`; Cohen's d=`r printnum(this.expt$H3$d)`; `r apa_print(this.expt$H3$BF)$statistic`). 

*Hypothesis 4*: In line with our hypothesis, response times for 'curved' responses were significantly faster than response times for 'straight' responses, with a median difference of `r e4$H4$raw_es%>%abs()%>%round()` ms (`r apa_print(this.expt$H4$t.test)$statistic` ; Cohen's d = `r printnum(this.expt$H4$d)`; `r apa_print(this.expt$H4$BF)$statistic`; see Figure \@ref(fig:all-rcROCs), panel 4).
...
In Experiment 5, we examined discrimination judgments between upward-tilted and downward-tilted cubes. Based on a search asymmetry for these stimuli [upward-tilted cubes are found faster among downward-tilted cubes than vice versa, in line with an expectation to see objects on the ground and not floating in space; @von1994visual], we hypothesized that a similar asymmetry would emerge in subjective confidence judgments, such that metacognitive sensitivity for perceiving an upward-tilted cube will be higher than for perceiving a downward-tilted cube. We used four dollar signs (\$\$\$\$) as our mask.

```{r e5, echo=FALSE, fig.show='hide', cache=TRUE}

e5=list()

e5$df <- read_csv('../experiments/Experiment5/data/jatos_results_batch1.csv') %>%
  rename('subj_id' = 'subject_identifier')

e5$export <- read_csv('../experiments/Experiment5/data/prolific_export_batch1.csv')  %>%
  rename('subj_id' = 'participant_id')
...
e5 <- e5%>%
  ...
  testH3() %>%
  testH4() %>%
  plotROC(c('up','down'),'figures/exp5.png')
...
Median completion time was `r printnum(this.expt$median_completion_time)` minutes. Mean accuracy was `r printnum(this.expt$disc_df_unfiltered$correct%>%mean())`, and participants reported seeing a downward-tilted cube on `r printnum(this.expt$disc_df_unfiltered$response%>%as.numeric()%>%mean()-1)` of the trials. Overall we excluded `r this.expt$N_total-this.expt$N` participants, leaving `r this.expt$N` participants for the main analysis. Going forward, only data from included participants is analyzed. 

Mean accuracy among included participants was `r apa_print(this.expt$disc_subj_stats$acc%>%t.test())$estimate`. The mean SOA of the last trial was `r apa_print(t.test(this.expt$final_trial$measured_SOA))$estimate`. Participants showed no consistent response bias (`r apa_print(this.expt$disc_subj_stats$bias%>%t.test())$estimate`). On a scale of 0 to 1, mean confidence level was `r apa_print(this.expt$disc_subj_stats$conf%>%t.test())$estimate`. Confidence was higher for correct than for incorrect responses (`r apa_print(this.expt$conf_by_acc)$full_result`). 

*Hypothesis 1*: Contrary to our hypothesis, confidence was similar for upward-tilted (feature present) responses and downward-tilted (feature absent) responses (`r apa_print(this.expt$H1$t.test)$full_result`; Cohen's d = `r printnum(this.expt$H1$d)`; `r apa_print(this.expt$H1$BF)$statistic`; see Figure \@ref(fig:all-rcROCs), panel 5).

*Hypothesis 2*: Contrary to our hypothesis, auROC for reports of seeing an upward-tilted cube (`r apa_print(t.test(this.expt$AUC$response1))$estimate`) was similar to auROC for reports of seeing a downward-tilted cube (`r apa_print(t.test(this.expt$AUC$response2))$estimate`; Cohen's d = `r printnum(this.expt$H2$d)`; `r apa_print(this.expt$H2$BF)$statistic`; see Figure \@ref(fig:all-rcROCs), panel 5.). In contrast with the Bayes Factor analysis, a t-test revealed a significant metacognitive asymmetry, with higher metacognitive sensitivity for perceiving an upward-tilted (default-violating) cube (`r apa_print(this.expt$H2$t.test)$statistic`).
...
*Hypothesis 4*: Contrary to our hypothesis, response times for 'upward-tilted' responses were similar to response times for 'downward-tilted' responses with a median difference of `r e5$H4$raw_es%>%abs()%>%round()` (`r apa_print(this.expt$H4$t.test)$statistic` ; Cohen's d = `r printnum(this.expt$H4$d)`; `r apa_print(this.expt$H4$BF)$statistic`; see Figure \@ref(fig:all-rcROCs), panel 5).
...
In Experiment 6, we examined discrimination judgments between flipped and normal *N* stimuli. Based on a search asymmetry for these stimuli [flipped *N*s are found faster among normal Ns than vice versa; @frith1974acurious; @wang1994familiarity], we hypothesized that a similar asymmetry would emerge in subjective confidence judgments, such that metacognitive sensitivity for perceiving a flipped *N* will be higher than for perceiving a normal *N*. We used four dollar signs (\$\$\$\$) as our mask.

```{r e6, echo=FALSE, fig.show='hide', cache=TRUE}

e6=list()

e6$df <- read_csv('../experiments/Experiment6/data/jatos_results_batch1.csv') %>%
  rename('subj_id' = 'subject_identifier')

e6$export <- read_csv('../experiments/Experiment6/data/prolific_export_batch1.csv')  %>%
  rename('subj_id' = 'participant_id')
...
e6 <- e6%>%
  ...
  testH3() %>%
  testH4() %>%
  plotROC(c('flipped','normal'),'figures/exp6.png')

this.expt <- e6;

```
`r this.expt$N_total` participants were recruited from Prolific for Experiment 6. 

Median completion time was `r printnum(this.expt$median_completion_time)` minutes. Mean accuracy was `r printnum(this.expt$disc_df_unfiltered$correct%>%mean())`, and participants reported seeing a normal *N* on `r printnum(this.expt$disc_df_unfiltered$response%>%as.numeric()%>%mean()-1)` of the trials. Overall we excluded `r this.expt$N_total-this.expt$N` participants, leaving `r this.expt$N` participants for the main analysis. Going forward, only data from included participants is analyzed. 

Mean accuracy among included participants was `r apa_print(this.expt$disc_subj_stats$acc%>%t.test())$estimate`. The mean SOA in the last trial was `r apa_print(t.test(this.expt$final_trial$measured_SOA))$estimate`. Participants showed no consistent response bias (`r apa_print(this.expt$disc_subj_stats$bias%>%t.test())$estimate`). On a scale of 0 to 1, mean confidence level was `r apa_print(this.expt$disc_subj_stats$conf%>%t.test())$estimate`. Confidence was higher for correct than for incorrect responses (`r apa_print(this.expt$conf_by_acc)$full_result`). 

*Hypothesis 1*: Contrary to our hypothesis, confidence was *lower* for flipped (feature present) responses than for normal (feature absent) responses. This result was in the opposite direction to what we had expected, so was not significant in a one-tailed t-test (`r apa_print(this.expt$H1$t.test)$full_result`; Cohen's d = `r printnum(this.expt$H1$d)`). However, a Bayes Factor favoured the alternative over the null (`r apa_print(this.expt$H1$BF)$statistic`; see Figure \@ref(fig:all-rcROCs), panel 6).

*Hypothesis 2*: Contrary to our hypothesis, auROC for reports of seeing a flipped *N* (`r apa_print(t.test(this.expt$AUC$response1))$estimate`) was similar to auROC for reports of seeing a normal *N* (`r apa_print(t.test(this.expt$AUC$response2))$estimate`; `r apa_print(this.expt$H2$t.test)$statistic`; Cohen's d = `r printnum(this.expt$H2$d)`; `r apa_print(this.expt$H2$BF)$statistic`; see Figure \@ref(fig:all-rcROCs), panel 6.). 

*Hypothesis 3*: (The lack of) metacognitive asymmetry was not different from what would be expected based on an equal-variance SDT model with the same response bias and sensitivity (`r apa_print(this.expt$H3$t.test)$statistic`; Cohen's d=`r printnum(this.expt$H3$d)`; `r apa_print(this.expt$H3$BF)$statistic`). 

*Hypothesis 4*: Contrary to our hypothesis, response times for 'flipped' responses were significantly *slower* than response times for 'normal' responses, with a median difference of `r e6$H4$raw_es%>%abs()%>%round()` ms. (`r apa_print(this.expt$H4$t.test)$statistic` ; Cohen's d = `r printnum(this.expt$H4$d)`; `r apa_print(this.expt$H4$BF)$statistic`; see Figure \@ref(fig:all-rcROCs), panel 6).
...
## Experiments 1-6: summary {#summary}
...
```{r summary, echo=FALSE, fig.cap= "Summary of results from Experiments 1-6, and exploratory Experiment 7. Rows correspond to our four pre-registered hypotheses: difference in confidence, a difference in metacognitive sensitivity, a difference in metacognitive sensitivity when controlling for response and confidence bias, and a difference in response times.", out.width = '100%'}
knitr::include_graphics("figures/summary.png")
```

## Experiment 7 (exploratory): grating vs. noise

Results from Experiments 1-6 revealed that search asymmetry is not always accompanied by an asymmetry in metacognitive sensitivity. Given that we did not observe a true metacognitive asymmetry in the expected direction for any of our stimulus pairs, we were concerned that our experimental design may have been unsuitable for detecting classical metacognitive asymmetries in detection, for example due to an insufficient number of trials, the masking procedure, or the confidence report scheme. As a positive control, we collected data for an additional experiment that more closely resembled typical detection experiments. In this experiment, participants discriminated between two stimuli: random noise and a noisy grating (presented to participants as a 'zebra' stimulus; see Fig. \@ref(fig:rcROC7)). In a previous lab-based study, similar stimuli produced a robust metacognitive asymmetry between target absent (noise) and target present (noisy grating) responses [@mazor2020distinct]. We used black and white concentric circles as a mask. Apart from the choice of stimuli and mask, the procedure was identical to that of our pre-registered experiments.
...
```{r e7, echo=FALSE, fig.show='hide', cache=TRUE}

e7=list()

e7$df <- read_csv('../experiments/Experiment7Grating/data/jatos_results_batch1.csv') %>%
  rename('subj_id' = 'subject_identifier')

e7$export <- read_csv('../experiments/Experiment7Grating/data/prolific_export_batch1.csv')  %>%
  rename('subj_id' = 'participant_id')
...
e7 <- e7%>%
  ...
  testH3() %>%
  testH4() %>%
  plotROC(c('present','absent'),'figures/exp7.png')

this.expt <- e7;

```

Median completion time was `r printnum(this.expt$median_completion_time)` minutes. Mean accuracy was `r printnum(this.expt$disc_df_unfiltered$correct%>%mean())`, and participants reported seeing a grating on `r printnum(this.expt$disc_df_unfiltered$response%>%as.numeric()%>%mean()-1)` of the trials. Overall we excluded `r this.expt$N_total-this.expt$N` participants, leaving `r this.expt$N` participants for the main analysis. Going forward, only data from included participants is analyzed. 

Mean accuracy among included participants was `r apa_print(this.expt$disc_subj_stats$acc%>%t.test())$estimate`. The mean SOA of the last trial was `r apa_print(t.test(this.expt$final_trial$measured_SOA))$estimate`. Participants showed no consistent response bias (`r apa_print(this.expt$disc_subj_stats$bias%>%t.test())$estimate`). On a scale of 0 to 1, mean confidence level was `r apa_print(this.expt$disc_subj_stats$conf%>%t.test())$estimate`. Confidence was higher for correct than for incorrect responses (`r apa_print(this.expt$conf_by_acc)$full_result`). 

*Hypothesis 1*: In line with our hypothesis, confidence was higher for reports of target presence than for reports of target absence (`r apa_print(this.expt$H1$t.test)$full_result`; Cohen's d = `r printnum(this.expt$H1$d)`; `r apa_print(this.expt$H1$BF)$statistic`; see Figure \@ref(fig:rcROC7), right panel).

*Hypothesis 2*: In line with our hypothesis, auROC for reports of target presence (`r apa_print(t.test(this.expt$AUC$response1))$estimate`) was significantly higher than for reports of target absence (`r apa_print(t.test(this.expt$AUC$response2))$estimate`; `r apa_print(this.expt$H2$t.test)$statistic`; Cohen's d = `r printnum(this.expt$H2$d)`; `r apa_print(this.expt$H2$BF)$statistic`; see Figure \@ref(fig:rcROC7), left panel). 
...
*Hypothesis 4*: In line with our hypothesis, reports of target presence were significantly faster than reports of target absence, with a median difference of `r e7$H4$raw_es%>%abs()%>%round` ms. (`r apa_print(this.expt$H4$t.test)$statistic` ; Cohen's d = `r printnum(this.expt$H4$d)`; `r apa_print(this.expt$H4$BF)$statistic`see Figure \@ref(fig:rcROC7), right panel).

```{r rcROC7, echo=FALSE, fig.cap= "Response conditional ROC curves (left panel) and confidence and reaction time distributions (right panel) for Exp. 7 (detection positive control). The strucutre of this figure is similar to Figs 3-4.", out.width = '100%'}
knitr::include_graphics("figures/results7.png")
```
...
```{r dropfewerrors, echo=FALSE, cache=TRUE}

filter_enough_errors <- function(e) {
  
  enough_errors_ss <- e$AUC %>%
    filter(min_per_cell>=5) %>%
    pull(subj_id);
  
  filtered_e <- e;
  
  filtered_e$df <- filtered_e$df %>% 
    filter(subj_id %in% enough_errors_ss)
  
  return(filtered_e)
}

filtered_eno_e1 <- e1 %>%
  filter_enough_errors() %>%
  filterDataFrame() %>%
  generalStats() %>%
  testH1() %>%
  testH2() %>%
...
<!-- Examining the response-conditional ROC curves, we noticed that some participants had response-conditional ROC curves that were defined by one or two points only (apart from the (0,0) and (1,1) points). In our study, this happened when participants made only a few errors of a certain type (for example, mistaking an *O* for a *Q*). This was the case for many of the feature-absent ROC curves in Experiments 3 and 4 (global features), due to a general bias to report seeing the feature-present stimulus in these experiments. Response-conditional ROC curves that are defined by only a few points are noisy and unreliable, and may have masked true effects of metacognitive asymmetry. To look into this, we repeated the analysis for Hypotheses 2 and 3 after ensuring that all participants had at least 5 errors of each type (instead of 2; included N = `r filtered_eno_e1$N`, `r filtered_eno_e2$N`, `r filtered_eno_e3$N`, `r filtered_eno_e4$N`, `r filtered_eno_e5$N`, `r filtered_eno_e6$N` for experiments 1-6, respectively). For this subset of our participants, the unexpected metacognitive asymmetry in Experiment 2 (*O*>*C*) was no longer supported (H2: `r filtered_eno_e2$H2$BF%>%apa_print()%>%'$'(statistic)`; H3: `r filtered_eno_e2$H3$BF%>%apa_print()%>%'$'(statistic)`). For all other experiments, this additional exclusion criterion did not change the qualitative pattern of our results.  -->
...
zROC slopes were numerically shallower than one in Experiments 1 (Q vs. O; median slope = `r e1$zROC_slopes$slope%>%median%>%printnum()`), 3 (line tilt; median slope = `r e3$zROC_slopes$slope%>%median%>%printnum()`), 4 (line curvature; `r e4$zROC_slopes$slope%>%median%>%printnum()`) and 5 (cube orientation; `r e5$zROC_slopes$slope%>%median%>%printnum()`). This was significant only in Experiment 5 (`r apa_print(e5$zROCtest$t.test)$statistic`). In agreement with the results of our rcROC analysis, the zROC slope in Exp. 2 (C vs. O) was significantly higher than one, suggesting that the representation of the letter 'O' was more variable than that of the letter 'C' (median slope = `r e2$zROC_slopes$slope%>%median%>%printnum()`; `r apa_print(e2$zROCtest$t.test)$statistic`). 
...
p1 <- all_data %>% 
  ...
  theme_minimal() +
  labs(title='Confidence and response time asymmetries', 
       x='\u0394 confidence', 
...
p2 <- all_data %>% 
  ...
  geom_point(data = all_data %>%
             ...
             size=5, colour='black', pch=21) + 
  theme_minimal() +
  labs(title='Confidence and metacognitive asymmetries', 
       x='\u0394 confidence', 
       y='\u0394 AUC',
       fill='Experiment')+
  guides(color=FALSE)+
...
# plot_grid(p1,p2,ncol=1, rel_heights=c(1,1.4))
knitr::include_graphics("figures/correlations.png")

```

<!-- ### Implicit metacognitive sensitivity as measured with RT -->

```{r rcROCRT, echo=FALSE, fig.cap="Example rcROC based on response times for Exp 1. The bowed ROCs suggest that RTs are reliably indicative of response accuracy. No implicit metacognitive asymmetry was observed for this stimulus pair.", cache=TRUE}

# testRTROC <- function(e) {
#   
#   e$RT_counts <- e$disc_df %>%
#     mutate(subj_id=factor(subj_id)) %>%
#     group_by(subj_id, response, correct, RT, .drop=FALSE) %>%
#     tally() %>%
#     spread(correct, n, sep='', fill=0) %>%
...
#            cs_incorrect=cumsum(correct0)/sum(correct0));
#   
#   e$RT_counts <-e$RT_counts %>%
#     group_by(subj_id, response,.drop=TRUE) %>%
#     summarise(
#       cs_correct=c(0,1),
#       cs_incorrect=c(0,1)) %>%
#     bind_rows(e$RT_counts,.) %>%
#     group_by(subj_id, response, cs_incorrect) %>%
#     summarise(cs_correct=max(cs_correct)) %>%
#     merge(e$disc_subj_stats%>%dplyr::select(subj_id,dprime, hit_rate, false_alarm_rate))%>%
#     mutate(miss_rate=1-hit_rate,
...
#         summarise(AUC = auc(cs_incorrect, cs_correct)) %>%
#         spread(response, AUC, sep='')%>%
#         mutate(metacognitive_RT_asymmetry=(response1-response2),
#                average_RT_AUC=response1/2+response2/2) %>%
#         rename(response1RT=response1, response2RT=response2),
#       by='subj_id')
#   
#   
#   e$RTROC$t.test <- t.test(e$AUC$metacognitive_RT_asymmetry);
#   
#   e$RTROC$d <- cohensD(e$AUC$metacognitive_RT_asymmetry);
#   
#   e$RTROC$BF <- ttestBF(
#     e$AUC$metacognitive_RT_asymmetry,
#     rscale=0.65);
#   
#   e$sdtRTAUC <- e$RT_counts %>%
#     group_by(subj_id, response,.drop=TRUE) %>%
#     summarise(AUC = auc(cs_incorrect, cs_correct_from_sdt)) %>%
#     spread(response, AUC, sep='')%>%
#     mutate(metacognitive_RT_asymmetry_from_sdt=(response1-response2))
#   
#   e$AUC <- e$AUC %>%
#     merge(e$sdtRTAUC%>%dplyr::select(subj_id,metacognitive_RT_asymmetry_from_sdt)) %>%
#     mutate(metacognitive_RT_asymmetry_control = metacognitive_RT_asymmetry-metacognitive_RT_asymmetry_from_sdt)
#   
#   e$contrast_df <- e$contrast_df %>%
#     merge(e$AUC%>%dplyr::select(subj_id, metacognitive_RT_asymmetry,metacognitive_RT_asymmetry_control, average_RT_AUC),
#           by='subj_id')
#   
#   e$RTROCcontrol$t.test <- e$AUC$metacognitive_RT_asymmetry_control %>%
#     t.test();
#   
#   e$RTROCcontrol$d <- e$AUC$metacognitive_RT_asymmetry_control %>%
#     cohensD();
#   
#   e$RTROCcontrol$BF <- e$AUC$metacognitive_RT_asymmetry_control %>%
#     ttestBF(rscale=0.65);
#   
#   return(e)
# }
# 
# 
# plotRTROC = function(e, labels, exp_number) {
#   
#   e$RT_discrete_counts <- e$disc_df %>%
#     mutate(subj_id=factor(subj_id),
#            RT_discrete = factor(ntile(RT,20))) %>%
#     group_by(subj_id, response, correct, RT_discrete, .drop=FALSE) %>%
#     tally() %>%
#     spread(correct, n, sep='') %>%
#     arrange(RT_discrete, by_group=TRUE) %>%
#     group_by(subj_id,response)%>%
#     mutate(cs_correct=cumsum(correct1)/sum(correct1),
#            cs_incorrect=cumsum(correct0)/sum(correct0))
#   
#   e$RT_discrete_counts_group <- e$RT_discrete_counts %>%
#     group_by(response, RT_discrete)%>%
#     summarise(RT_incorrect = mean(cs_incorrect, na.rm=TRUE),
#               RT_correct = mean(cs_correct, na.rm=TRUE),
#               RT_incorrect_sem = se(cs_incorrect, na.rm=TRUE),
#               RT_correct_sem  = se(cs_correct, na.rm=TRUE))
#   
#   
#   e$rcRTROC <- ggplot(data=e$RT_discrete_counts_group%>%mutate(response=ifelse(response==1,labels[1],labels[2])%>%factor(levels=labels)),
#                       aes(x=RT_incorrect, y=RT_correct, color=response)) +
#     geom_line(size=1) +
#     geom_point(aes(shape = response))+
#     geom_errorbar(aes(ymin = RT_correct-RT_correct_sem,ymax = RT_correct+RT_correct_sem)) +
#     geom_errorbar(aes(xmin = RT_incorrect-RT_incorrect_sem,xmax = RT_incorrect+RT_incorrect_sem)) +
#     geom_abline(slope=1)+
#     theme_bw() + coord_fixed() +
#     labs(x='p(RT | incorrect)', y='p(RT | correct)', title=paste('rcROC curves for RT (Exp. ', exp_number,')'))+ 
#     scale_color_manual(values=wes_palette(n=3, name="Darjeeling1"))+
#     scale_fill_manual(values=wes_palette(n=3, name="Darjeeling1")) +
#     geom_rect(aes(xmin=0,xmax=1,ymin=0,ymax=1),size=0.5,color='black',alpha=0)+
#     geom_line(data=e$RT_counts %>%
#                 filter(as.integer(subj_id)<20) %>%
#                 mutate(response=ifelse(response==1,labels[1],labels[2]) %>%
#                          factor(levels=labels)),
...
#   
#   e$RTAUClong <- e$AUC %>%
#     gather('response','rcAUC',which(e$AUC%>%colnames()=='response1RT'):which(e$AUC%>%colnames()=='response2RT')) %>%
#     mutate(response=ifelse(response=='response1RT',labels[1],labels[2])%>%factor(levels=labels))
#   
#   e$RTAUCplot <- ggplot(e$RTAUClong,aes(x=response, y=rcAUC,group=subj_id))+
...
#   
#   ggdraw(e$rcRTROC)+
#     draw_plot(e$RTAUCplot,0.55,0.13,0.2,0.5,hjust=0, vjust=0)
#   
# }
# 
# 
...
# e6 <- testRTROC(e6);
# 
# #to integrate the new data about RT
# all_data <- rbind(
#   e1$contrast_df%>%mutate(exp=1),
#   e2$contrast_df%>%mutate(exp=2),
#   e3$contrast_df%>%mutate(exp=3),
...
# formatForConfidenceDatabase <- function(df,stimuli,exp) {
#   df %>%
#     filter(test_part=='disc') %>%
#     dplyr::select('subj_id','which_stimulus','SOA','correct','RT','confidence','conf_RT') %>%
#     group_by(subj_id) %>%
#     mutate(trial=row_number())%>%
#     ungroup()%>%
#     mutate(subj_id=factor(subj_id),
#            Subj_idx = as.numeric(subj_id),
#            Stimulus=stimuli[which_stimulus],
#            Response=ifelse(correct==1,
#                            stimuli[which_stimulus],
#                            stimuli[3-as.numeric(which_stimulus)]),
#            RT_dec=RT,
#            RT_conf=conf_RT,
#            Accuracy=correct,
#            Trial=trial)%>%
#     select(Subj_idx, Stimulus, Response, RT_dec, RT_conf, SOA, Accuracy, Trial)%>%
#     arrange(Subj_idx, Trial)%>%
#     write.csv(.,file=paste('..\\ConfidenceDataBase\\data_Mazor_2021_Expt',exp,'.csv',sep=''),row.names=FALSE)
# }
# 
# formatForConfidenceDatabase(e1$df,c('Q','O'),1);
# formatForConfidenceDatabase(e2$df,c('C','O'),2);
# formatForConfidenceDatabase(e3$df,c('tilted','vertical'),3);
# formatForConfidenceDatabase(e4$df,c('curved','straight'),4);
# formatForConfidenceDatabase(e5$df,c('up','down'),5);
# formatForConfidenceDatabase(e6$df,c('flipped','normal'),6);
# formatForConfidenceDatabase(e7$df,c('grating','noise'),7);
