...
e1.all_subjects <- e1.df$subj_id%>%unique()

e1.N_total <- e1.all_subjects%>%length()

e1.disc_df_unfiltered <- e1.df %>%
...
##exclude subjects
e1.bad_disc <- e1.disc_df_unfiltered %>%
  group_by(subj_id) %>%
...
e1.failed_test <- e1.df %>%
  group_by(subj_id) %>%
  summarise(fi=all(followed_instructions)) %>%
  filter(fi==FALSE) %>%
  select('subj_id') 

e1.excluded <- union(e1.failed_test, e1.bad_disc)%>%pull()

e1.disc_df_half_filtered <- e1.disc_df_unfiltered %>%
  filter(!subj_id%in%e1.excluded & 
           RT>minRT & 
           RT<maxRT) %>%
  group_by(subj_id) %>%
  mutate(
...
e1.not_enough_errors <- e1.disc_df_half_filtered %>%
...
e1.no_variance <- e1.disc_df_half_filtered %>%
...
e1.excluded2 <- union(e1.no_variance,e1.not_enough_errors)%>%pull()
e1.included <- setdiff(setdiff(e1.all_subjects,e1.excluded),e1.excluded2)

e1.disc_df <- e1.disc_df_half_filtered %>%
  filter(subj_id %in% e1.included)

e1.N <- e1.included%>%length()
```

`r N_total` participants were recruited from Prolific for Experiment 1.

Mean accuracy was `r e1.disc_df_unfiltered$correct%>%mean()`. We excluded participants for performing below `r min_acc_disc*100`% accuracy, for having extremely fast or slow reaction times (below `r minRT` milliseconds or above `r maxRT/1000` seconds in more than 25% of the trials), and for failing the comprehension check. Overall we excluded `r e1.N_total-e1.N` participants, leaving `r e1.N` participants for the main analysis.
...
e1.disc_subj_stats <- e1.disc_df %>%
  ...
  summarise(
    ...
    RT=mean(RT),
    SOA=mean(SOA))%>%
  merge(e1.export,by='subj_id')

# TO REPORT SOA ON FINAL TRIAL
e1.final_trial <- e1.disc_df%>%filter(trial==96);
...
e1.conf_counts <- e1.disc_df %>%
  mutate(subj_id=factor(subj_id)) %>%
  group_by(subj_id, response, correct, confidence, .drop=FALSE) %>%
  tally() %>%
  spread(correct, n, sep='', fill=0) %>%
  merge(e1.disc_subj_stats%>%select(subj_id,dprime, hit_rate, false_alarm_rate))%>%
...
e1.AUC <- e1.AUC %>%
  merge(e1.sdtAUC%>%select(subj_id,metacognitive_asymmetry_from_sdt)) %>%
          mutate(metacognitive_asymmetry_control = metacognitive_asymmetry-metacognitive_asymmetry_from_sdt)

e1.disc_subj_stats <-merge( e1.disc_subj_stats,
                         e1.AUC) %>%
...
Mean accuracy in the discrimination task was `r mean(e1.disc_subj_stats$acc)` (`r apa_print(e1.disc_subj_stats$acc%>%t.test())$estimate`). mean SOA in the 32th trial was `r apa_print(t.test(e1.final_trial$SOA))$estimate`. Participants showed no consistent bias in their responses (`r apa_print(e1.disc_subj_stats$bias%>%t.test())$estimate`). On a scale of 0 to 1, mean confidence level was `r mean(e1.disc_subj_stats$conf)` (`r apa_print(e1.disc_subj_stats$conf%>%t.test())$estimate`). Confidence was higher for correct than for incorrect responses (`r apa_print(e1.conf_by_acc)$full_result`). 

*Hypothesis 1*: In line with our hypothesis, confidence was generally higher for *Q* (feature present) responses than for *O* (feature absent) responses (`r apa_print(e1.conf_by_resp)$full_result`; Cohen's d = `r printnum(e1.conf_by_resp_d)`).

*Hypothesis 2*: In order to measure metacognitive asymmetry, we extracted the response-conditional type-2 ROC (rc-ROC) curves for the two responses (*Q* and *O*) in the discrimination task. This was done by plotting the cumulative distribution of confidence ratings (high to low) for correct responses against the same distribution for incorrect responses. The area under the rc-ROC curve (auROC) was then taken as a measure of metacognitive sensitivity [@kanai2010subjective; @meuwese2014subjective]. In line with our hypothesis, auROC for *Q* responses (`r apa_print(t.test(AUC$response1))$estimate`) was higher than for *O* responses (`r apa_print(t.test(e1.AUC$response2))$estimate`; `r apa_print(t.test(e1.AUC$response1, e1.AUC$response2, paired=TRUE))$statistic`; Cohen's d = `r printnum(cohensD(e1.AUC$response1, e1.AUC$response2,method='paired'))`; see Figure \@ref(fig:rc-ROC-pilot)), mirroring the metacognitive asymmetry for detection judgments. 

*Hypothesis 3*: Metacognitive asymmetry was significantly higher than what would be expected based on an equal-variance SDT model with the same response bias and sensitivity (`r apa_print(t.test(e1.AUC$metacognitive_asymmetry_control))$statistic`, Cohen's d=`r printnum(cohensD(e1.AUC$metacognitive_asymmetry_control))`).

*Hypothesis 4*: In line with our hypothesis, *Q* responses were faster on average than *O* responses (`r apa_print(e1.logRT_by_resp)$statistic` ; Cohen's d = 
`r printnum(e1.logRT_by_resp_d)`). 
...
e1.conf_discrete_counts <- e1.disc_df %>%
  mutate(subj_id=factor(subj_id),
...
e1.conf_discrete_counts_group <- e1.conf_discrete_counts %>%
...
e1.rcROC <- ggplot(data=e1.conf_discrete_counts_group%>%mutate(response=ifelse(response==1,'Q','O')%>%factor(levels=c('Q','O'))),
...
e1.AUCplot <- ggplot(e1.AUClong,aes(x=response, y=rcAUC,group=subj_id))+
  geom_line()+geom_point(aes(colour=response), size=2, fill="white", show.legend = FALSE)

ggdraw(e1.rcROC)+draw_plot(e1.AUCplot,0.55,0.13,0.2,0.5,hjust=0, vjust=0)

```

```{r zROC, echo=FALSE, eval=FALSE}
# ZROC ANALYSIS
