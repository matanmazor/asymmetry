---
title             : "Metacognitive and search asymmetries for feature presence and absence"
...
abstract: |
  Subjective confidence judgments are less aligned with objective performance following decisions about the absence, rather than the presence of a target object. This metacognitive asymmetry between decisions about presence and absence is one instance of a general difficulty to infer, represent, and attend to absence. In visual search, the presence/absence asymmetry has been demonstrated not only for the presence or absence of the target object, but also of stimulus features. Using stimulus pairs that have been shown to induce asymmetries in visual search, here we ask whether similar sub-stimulus presence/absence asymmetries will be revealed for metacognitive evalutations of perceptual discrimination . 
...
Contextual effects have led @treisman1988feature to further propose that search asymmetry may sometimes reflect a deviation from a default state (also termed null, prototypical, or standard state). This default state is partly hard-coded into the wiring of the system, and partly learned and change according to context. For example, the *Q*-in-*O*/*O*-in-*Q* asymmetry is evident already at 3 months of age, suggesting that it corresponds to a core feature of the perceptual system [@colombo1995visual; @adler2014search]. Other aspects of the default state are sensitive to context and prior experience; for example, searching for a tilted line among vertical distractors is easier than the inverse search, but this effect reverses when the search array is displayed inside a tilted frame [@treisman1988feature]. An example for experience-driven asymmetry is that searching for an inverted letter among canonically presented letters is easier than the inverse search [@frith1974acurious; @wang1994familiarity]. This similar effect for feature presence and deviation from a default state is consistent with the idea that search asymmetry reveals a true asymmetry in our cognitive modeling of the world, where absence is assumed as default unless evidence is available for the contrary.  
...
To date, this feature-level presence/absence asymmetry has only been demonstrated in the visual search domain. Here we ask whether a similar feature-level asymmetry is evident in another domain where presence/absence asymmetry has been documented at the object level: metacognition for visual detection judgments. Lower metacognitive accuracy for negative responses has been observed not only for detection tasks, but also for recognition memory [@higham2009investigating] and lexicality judgments [@fleming2010effects] -- both can be described as detection among items in an internal representation space (memory storage, orthographic lexicon). According to one interpretation, this metacognitive asymmetry reflects a qualitative difference between the metacognitive representations of decisions that are driven by the presence or the absence of evidence [@meuwese2014subjective]. One variant of this interpretation posits that the metacognitive evaluation of negative judgments is more computationally demanding because instead of relying on evidence for absence, it relies on the counterfactual likelihood of evidence for presence [contrast "I saw it was not there" with "If it was there I would not have missed it"; @mazor2020distinct]. Crucially, metacognitive asymmetry is not observed for discrimination judgments about the identity of the stimulus, where evidence can be available in support of both responses. For example, when asking participants to decide whether it was a fish or a different animal that appeared in a degraded image, confidence ratings for the two responses are similarly indicative of true objective accuracy [@meuwese2014subjective].
...
Here, we propose to measure participants' metacognitive evaluations of discrimination judgments between a selected set of stimulus pairs that exhibit asymmetry in visual search. By doing so, we aim to tackle two questions. First, looking at individual and stimulus-related differences in the two measures will allow us to directly test whether search and metacognitive asymmetries originate from identical, partly overlapping, or completely distinct underlying mechanism. Second, with confidence ratings in single-item discrimination we will get a measure of the relative variability for the two stimulus types that is independent of visual search. This measure can then inform and constrain theories of search asymmetry, and more specifically support or rule out theories that rely on unequal-variance assumptions to explain search time differences.
...
This hypothesis will be tested for the following stimulus features for which search asymmetry has been demonstrated: addition of a stimulus part, open ends, color, curvature, orientation and familiarity. We will test Hypothesis 1 separately for each stimulus feature, by replicating the original search asymmetry (for example, Q-in-O search is more efficient that O-in-Q search) and testing the null hypothesis that metacognitive sensitivity (measured as the area under the response conditional ROC curve) for the efficient target (e.g., *Q*) is equal to metacognitive sensitivity for the inefficient target (e.g., for the case of *Q*/*O*, $H_o=auROC_{Q}\leq auROC_O$). 
...
2. **Hypothesis 2**: Individual differences in the direction and magnitude of search asymmetry positively covary with individual differences in the direction and magnitude of metacognitive asymmetry. 

This hypothesis will be tested separately for each experiment by measuring the correlation between search asymmetry and metacognitive asymmetry across participants. We will also measure the overall correlation between search and metacognitive asymmetries across participants, pooled from all 6 experiments. 
...
3. **Color**. Red RGB(255,0,0) and magenta RGB(255,0,100) dots will be used as stimuli [@treisman1988feature]. 
...
5. **Familiarity**. Reversed and normal *N* will be used as stimuli [@frith1974acurious; @wang1994familiarity].
...
The area between the two curves is a measure of metacognitive asymmetry [@meuwese2014subjective]. This difference will be used to test Hypothesis 1.

2. The difference between search slopes for the two visual search tasks (defined as $\beta_1$ in the linear regression model $RT=\beta_0+\beta_1set\_size$, collapsed across both 'yes' and 'no' responses): $\Delta_s$. 

The difference between the curves is a measure of search asymmetry. This difference will be used to test Hypothesis 1. 

3. The relative difference between the area under the response-conditional type-2 ROC curves for the two responses ($\Delta^{rel}_m$) will be defined as the area between the two curves, divided by the sum of the areas under the two curves. 

This relative difference will be used to test Hypotheses 2 and 3.

4. The relative difference between search slopes for the two visual search tasks ($\Delta^{rel}_s$) will be defined as the difference between the two search slopes, divided by their sum. 
...
For each of the six experiments, Hypothesis 1 (metacognitive asymmetry follows the same direction as search asymmetry) will be tested using a two tailed t-test at the group level with $\alpha=0.05$. In case of a null result, a two one-sided test [TOST procedure; @schuirmann1987comparison] will be adopted to test the null hypothesis that $|\Delta_m|$ is higher than the smallest effect size of interest (0.05 in auROC units, ranging from 0 to 1), or that $|\Delta_s|$ is higher than the smallest effect size of interest (1 millisecond/item). 

Hypothesis 2 will be tested for each experiment by testing the correlation between $\Delta^{rel}_m$ and $\Delta^{rel}_s$, across participants. We will also test the correlation between $\Delta^{rel}_m$ and $\Delta^{rel}_s$ pooled across participants from all 6 experiments, and controlling for experiment-specific intercept effects. In case of a null result, a two one-sided test [TOST procedure; @schuirmann1987comparison] will be adopted to test the null hypothesis that $|r(\Delta^{rel}_m, \Delta^{rel}_s)|$ is higher than the smallest effect size of interest (0.05). 
...
1. Hypothesis 1: With 320 participants, we will have a statistical power of 95% to detect effects of size `r pwr.t.test(power=0.95,n=318,sig.level=0.05, type='paired')%>%'$'(d)`, which is less than half the effect size we observed for metacognitive asymmetry and less than fifth the effect size we observed for search slopes in our pilot sample. 

2. Hypothesis 2: For each stimulus pair, will have a statistical power of 95% to detect a correlation of `r pwr.r.test(power=0.95,n=318,sig.level=0.05)%>%'$'(r)` between search and metacognitive asymmetries. Across all six stimulus pairs, we will have a statistical power of 95% to detect a correlation of `r pwr.r.test(power=0.95,n=318*6,sig.level=0.05)%>%'$'(r)`. 
...
|-----------------------------------------------------------------------------------------------|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| Does metacognitive asymmetry extend to sub-stimulus features?                                 | Metacognitive asymmetry, measured as the area between the two response conditional ROC curves, follows the same direction as search asymmetry.                                     | 320 included participants in each experiment $\times$ 6 experiments. We will have 95% power to detect an effect size of 0.2.                                                                                            | T test on $\Delta_m$ against 0.  IF p>0.05, TOST procedure with SESOI=0.05.                                                                                                                                                                           | For each experiment separately, a significant result in the expected direction would be interpreted as metacognitive asymmetry for the specific stimulus feature.  Across all experiments, a series of significant results in the expected direction would be interpreted as supporting a common underlying mechanism for search and metacognitive asymmetries.  A significant TOST result would be interpreted as indicating indistinguishable metacognitive sensitivity for the two stimulsu types. |
| Is there a common source to metacognitive and search asymmetries?                             | Individual differences in the direction and magnitude of search asymmetry positively covary with individual differences in the direction and magnitude of metacognitive asymmetry. | 320 included participants in each experiment $\times$ 6 experiments. We will have 95% power to detect a correlation of 0.2 at the single experiment level, and a correlation of 0.08 pooled across all six experiments. | Pearson correlation between $\Delta^{rel}_m$ and $\Delta^{rel}_s$ at the single experiment level, and pooled across all six experiments after regressing out the effect of experiment on both measures.  IF p>0.05, a TOST procedure with SESOI=0.05. | For each experiment separately, a significant correlation would be interpreted as a common cause for the two asymmetries for the particular stimulus pair. Pooled across all six experiments, a significant correlation would be interpreted as reflecting a common cause for search and metacognitive asymmetries generally. A significant TOST result would be interpreted as evidence against a common mechanism underlying the two asymmetries.                                                   |
| Metacognitive and search asymmetries reflect unequal variance for selected stimulus features. | The positive association between search- and metacongitive- asymmetries is mediated by the sigma-ratio, extracted from confidence ratings in the discrimination task.              | 320 included participants in each experiment $\times$ 6 experiments. We will have 97% power to reject the null hypothesis for a specific experiment if the $r(Q_\sigma,\Delta^{rel}_m)$=0.2.                            | Contingent on a significant result for Hypothesis 2: Hotelling-Williams test for dependent correlations between $r(\Delta^{rel}_s,\Delta^{rel}_m)$ and $r(\Delta^{rel}_s,\Delta^{rel}_m.Q_\sigma)$.                                                   | For each experiment separately, a significant difference between the full and part correlations would be interpreted as evidence for the unequal-variance account for search- and metacognitive- asymmetries. A significant TOST result would be interpreted as supporting an alternative mechanism.                                                                                                                                                                                                  |
