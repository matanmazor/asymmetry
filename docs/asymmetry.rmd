...
These three interpretation presented here may not be entirely mutually exclusive. As pointed out by @treisman1988feature, and in line with Weber's law and with the noise profile of biological neurons, the positive representation of features and objects may be more variable, or noisy, than the representation of their absence. The unequal variance identified by signal detection models may be based in this qualitative differnce between the two classes of representation. Furthermore, a prallel search model may account for search asymmetry for presentation times below the time taken to complete a saccade, but an additional level of asymmetry, governed by endogeneous attention mechanisms, may be necessary to account for search times in natural displays that are not limited in time. Indeed, while the parallel models of @vincent2011search and @dosher2004parallel draw a link between unequal variance and asymptotic accuracy for short display times, they do not provide an explanation for the difference in search time and search slopes for long presentation times. Finally, it is not clear that all search asymmetries reflect the same underlying mechanism. While search asymmetry between moving and stationary items is amenable to Rosenholtz' argument about background interference, it is unclear how this explanation would account for the effects of inverted-letters or curvature asymmetries.
...
To date, this feature-level presence/absence asymmetry has only been demonstrated in the visual search domain, making it difficult to decide between alternative explanations. Here we ask whether a similar feature-level asymmetry is evident in three other domains where presence/absence asymmetry has been documented at the object level: response inhibition, metacognition, and working memory. By triagulating the feature-level asymmetry across six features and four tasks, we aim to obtain a better understanding of feature-level asymmetries, and to determine whether these asymmetries originate from identical, partly overlapping, or completely distinct mechanisms. We now turn to motivate our choice of experimental tasks, before describing our experimental design in detail in the methods section.

## Confidence in visual detection

In a typical detection task, participants are asked to decide whether a faint stimulus was present or absent. Subjective confidence ratings following 'target absent' judgments are commonly lower, and less correlated with objective accuracy, than following 'target present' judgments [@kanai2010subjective; @meuwese2014subjective; @kellij2018foundations; @mazor2020distinct]. This *metacognitive asymmetry* is present not only for detection tasks, but also for recognition memory [@higham2009investigating] and lexicality judgments [@fleming2010effects] -- both can be described as detection among items in an internal representation space (memory storage, orthographic lexicon). According to one interpretation, this metacognitive asymmetry reflects a qualitative difference between the metacognitive representations of decisions that are driven by the presence or the absence of evidence [@meuwese2014subjective]. One variant of this interpretation posits that the metacognitive evaluation of negative judgments is more computationally demanding because instead of relying on evidence for absence, it relies on the counterfactual likelihood of evidence for presence [contrast "I saw it was not there" with "If it was there I would not have missed it"; @mazor2020distinct]. In contrast, this asymmetry can also be interpreted as reflecting an asymmetry in the perceptual and cognitive uncertainties that are associated with the representations of presence and absence. For example, @kellij2018foundations successfully replicated the metacognitive asymmetry effect in a simple Signal Detection model with higher variance for the signal distribution. Their model did not have to posit any asymmetry in higher-order processes in order to fit the empirical data.

Crucially for our study, metacognitive asymmetry is not observed for discrimination judgments, where a decision is made about the identity of the stimulus instead of it presence. For example, when asking participants to decide whether it was a fish or a different animal that appeared in a degraded image, confidence ratings for the two responses are similarly indicative of true objective accuracy [@meuwese2014subjective]. It is unclear whether a metacognitive asymmetry is expected for discrimination judgments between stimulus features that show a search-asymmetry. Thus, in the context of our investigation, asking for confidence judgments following discrimination judgments for our selected stimuli will serve two purposes. First, it will allow us to directly test the hypothesis that search and metacognitive asymmetry originate from the same underlying mechanism by looking at individual and stimulus-related differences in the two measures. Second, with confidence ratings in discrimination we will get a measure of the relative variability for the two stimulus types that is independent of visual search. This measure can then inform and constrain theories of search asymmetry, for example, support or rule out theories that rely on unequal-variance assumptions to explain search time differences.
...
<!-- In our demo simulation, high-variance stimuli are associated with variance of 1, and low variance stimuli are associated with variance 0.8. Our agent positioned their threshold and starting point to minimize average response time, while maintaining accuracy > 0.95. For these stimulus-specific variance values, this means setting the threshold to 0.7 and the bias to 0.3. -->
 ...
 Searching for one *Q* among 10 *O*'s is easier than searching for one *O* among 10 *Q*s.  
