...
abstract: |
  Human and animal subjects generally find it more difficult to infer, represent, and attend to absence than to presence. In visual search, the presence/absence asymmetry has been demonstrated not only for the presence or absence of entire objects, but also of stimulus features. Furthermore, search asymmetry has also been found for familiarity and prototypicality, suggesting a more general asymmetry between default-complying and deviating stimuli. Using stimulus pairs that have been shown to induce asymmetries in visual search in conjunction with a metacognitive asymmetry for detection judgments, here we ask whether these feature-level asymmetries reflect one homogeneous effect or several different ones.
...
These perceptual and cognitive presence/absence asymmetries may reflect a profound asymmetry in the cognitive representational space, or alternatively a mixture of apparently similar but cognitively unrelated psychological effects. Although such asymmetries are consistently observed across tasks and stimulus features, to date no systematic attempt has been made to investigate whether these similar effects share a common underlying mechanism, or not. Furthermore, it remains a theoretical conjecture whether the presence/absence asymmetry is one instance of a more general perceptual and cognitive asymmetry between default and deviating states in a default-reasoning framework, where absence is only one among many aspects of the default state. Here we take a first step toward answering these two questions by testing the generalizability across domains of a well-documented presence/absence asymmetry in visual search for local features, global features, and for comliance with a default state.  

In visual search, participants report whether a target stimulus appeared or not among distractor stimuli in a search array. Participants typically take longer to identify the absence compared with the presence of a target, even in searches that are not performed serially, and even when the search array appears for a shorter period of time than required to perform eye movements [@wolfe1998can; @dosher2004parallel]. Furthermore, searching for an object that is marked by a distinguishing feature (for example, searching for a *Q* among *O*s) is more efficient than searching for an object that is marked by the absence of a distinguishing feature [searching for an *O* among *Q*s; @treisman1985search]. Such *visual search asymmetries* are not limited to the presence or absence of local stimulus features like the line that distinguishes a *Q* from an *O*. In a seminal paper, @treisman1988feature observed similar search asymmetries for the presence or absence of global stimulus features, like color, tilt, and curvature. For example, the asymmetry between magenta-in-red and red-in-magenta searches was explained as resulting from blue being present in magenta but absent in red. In the same way curvature and tilt were identified as positive visual features based on search time benefits for tilted-in-vertical and curved-in-straight searches, compared with the inverse searches. Furthermore, cases of search asymmetry for familiar and unfamiliar objects supported a default-compliance interpretation of the search asymmetry effect. For example, searching for an inverted letter among canonically presented letters and searching for a upward-tilted cube among downward-tilted cubes are easier than the inverse searches, in line with a general expectation to see letters in their canonical form, and objects on the ground rather than floating in space [@frith1974acurious; @wang1994familiarity; @von1994visual]. It is unclear however whether local, global, and default-compliance search-asymmetry effects originate from the same underlying mechanism.
...
Here, we propose to measure participants' metacognitive evaluations of discrimination judgments between a selected set of stimulus pairs that exhibit asymmetry in visual search for different perceptual dimensions at three levels: the presence or absence of a local feature, the presence or absence of a global feature, and comliance with a default state. For local features, we will test part addition (*Q*/*O*) and part subtraction (*C*/*O*). For global features, we will test orientation (vertical/tilted) and curvature (straight/curved). Finally, for default-compliance, we will test letter inversion and viewing angle (upward and downward tilted cuves). Looking at individual and stimulus-related differences in search and metacognitive asymmetries will allow us to directly test whether search and metacognitive asymmetries originate from common, partly overlapping, or completely distinct underlying mechanisms, and to do so separately for local features, global features, and default-compliance. 
...
1. **Hypothesis 1**: For our selected stimulus pairs, visual search is more efficient in one direction compared with the other.

This hypothesis will be tested for the following stimulus features for which search asymmetry has been demonstrated: addition of a stimulus part, open ends, orientation, curvature, letter inversion, and viewing angle. We will test Hypothesis 1 separately for each stimulus feature, by replicating the original search asymmetry (for example, Q-in-O search is more efficient that O-in-Q search).

1. **Hypothesis 2**: Conditioned on the results of our testing of Hypothesis 1. Metacognitive asymmetry, measured as the area between the two response conditional ROC curves (see methods section below), follows the same direction as search asymmetry. 
...
2. **Hypothesis 3**: Individual differences in the direction and magnitude of search asymmetry positively covary with individual differences in the direction and magnitude of metacognitive asymmetry. 
...
2. **Hypothesis 4**: Individual differences in the direction and magnitude of search asymmetry positively covary with individual differences in the additional time cost for reporting target absence in visual search. 
...
3. **Curvature**. Straight and curved lines will be used as stimuli [@treisman1988feature].
4. **Orientation**. Vertical and tilted lines will be used as stimuli [@treisman1988feature].
5. **Letter Inversion**. Reversed and normal *N* will be used as stimuli [@frith1974acurious; @wang1994familiarity].
6. **Viewing Angle**. Upward and Downward tilted cubes will be used as stimuli [@von1994visual].
...
Response conditional ROC curves will be extracted, by plotting the empirical cumulative distribution of confidence ratings for correct responses against the same cumulative distribution for incorrect responses. This will be done separately for the two responses, resulting in two curves. The area under the response-conditional ROC curve is a measure of metacognitive sensitivity. The area between the two curves is a measure of metacognitive asymmetry [@meuwese2014subjective]. This difference will be used to test Hypothesis 1.

2. The difference between search slopes for the two visual search tasks: $\Delta_s$. 

For each visual search task, the slope will be extracted for target present and target absent trials separately, and the average value will be taken. The difference between these average slopes is a measure of search asymmetry. This difference will be used to test Hypothesis 2. 
...
This relative difference will be used to test Hypothesis 3.
...
For each visual search response ('present' and 'absent'), the slope will be extracted for the two stimulus assignments separately, and the average value will be taken. The relative difference between search slopes for the two visual search responses ($\Delta^{rel}_{r}$) will be defined as the difference between the two search slopes, divided by the sum of their absolute values. 

This relative difference will be used to test Hypothesis 4.
...
For each of the six experiments, Hypothesis 1 (search asymmetry replication) will be tested using a two tailed t-test at the group level with $\alpha=0.05$.
...
Conditioned on that search asymmetry will be replicated, for each of the six experiments, Hypothesis 2 (metacognitive asymmetry follows the same direction as search asymmetry) will be tested using a one tailed t-test at the group level with $\alpha=0.05$.
...
Hypothesis 3 will be tested for each experiment by testing the Pearson correlation between $\Delta^{rel}_m$ and $\Delta^{rel}_s$, across participants. We will also test the Pearson correlation between $\Delta^{rel}_m$ and $\Delta^{rel}_s$ pooled across participants from all 6 experiments, and controlling for experiment-specific intercept effects. 

Hypothesis 4 will be tested for each experiment by testing the Pearson correlation between $\Delta^{rel}_s$ and $\Delta^{rel}_r$, across participants. We will also test the Pearson correlation between $\Delta^{rel}_s$ and $\Delta^{rel}_r$ pooled across participants from all 6 experiments, and controlling for experiment-specific intercept effects. 

For hypotheses 1 and 2, in case of a null result, a Bayes factor will be computed using the BayesFactor R package [@morey2015package] and using a Jeffrey-Zellner-Siow Prior with a medium rscale parameter of $\sqrt{2}/2$. For hypotheses 3 and 4, in case of a null result, the BayesFactor package will be used to compute a Bayes factor using a shifted, scaled beta(3,3) prior distribution for the correlation. 
...
1. Hypothesis 1: With 320 participants, we will have a statistical power of 95% to detect effects of size `r pwr.t.test(power=0.95,n=318,sig.level=0.05, type='paired')%>%'$'(d)`, which is less than fifth the effect size we observed for search slopes in our pilot sample. 

2. Hypothesis 2: With 320 participants, we will have a statistical power of 95% to detect effects of size `r pwr.t.test(power=0.95,n=318,sig.level=0.05, type='paired', alternative='greater')%>%'$'(d)`, which is less than half the effect size we observed for metacognitive asymmetry in our pilot sample. 

3. Hypothesis 3: For each stimulus pair, will have a statistical power of 95% to detect a Pearson correlation of `r pwr.r.test(power=0.95,n=318,sig.level=0.05)%>%'$'(r)` between search and metacognitive asymmetries. Across all six stimulus pairs, we will have a statistical power of 95% to detect a Pearson correlation of `r pwr.r.test(power=0.95,n=318*6,sig.level=0.05)%>%'$'(r)`. 

4. Hypothesis 4: For each stimulus pair, will have a statistical power of 95% to detect a Pearson correlation of `r pwr.r.test(power=0.95,n=318,sig.level=0.05)%>%'$'(r)` between search type and search response asymmetries. Across all six stimulus pairs, we will have a statistical power of 95% to detect a Pearson correlation of `r pwr.r.test(power=0.95,n=318*6,sig.level=0.05)%>%'$'(r)`. 
