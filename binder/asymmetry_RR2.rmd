...
We report how we determined our sample size, all data exclusions (if any), all manipulations, and all measures in the study. <!-- 21-word solution (Simmons, Nelson & Simonsohn, 2012; retrieved from http://ssrn.com/abstract=2160588) --> The full registered protocol is available at osf.io/ed8n7.
...
```{r trialstructure, echo=FALSE, fig.cap= "Experiment design. Metacognitive asymmetry effects were tested for six stimulus features in six separate experiments, encompassing three levels of abstraction: local features, global features, and expectation violations. The presented trial corresponds to the first stimulus pair, with *Q* and *O* as the two stimuli.", out.width = '100%'}
...
filterDataFrame <- function(e) {
  ...
  excluded2 <- e$not_enough_errors%>%pull()
  
  e$included <- setdiff(setdiff(e$all_subjects,excluded),excluded2)

  disc_df_almost_filtered <- disc_df_half_filtered %>%
    filter(subj_id %in% e$included);
  
  e$no_variance <- disc_df_almost_filtered %>%
  ...
  e$disc_df <- disc_df_almost_filtered %>%
    filter(!subj_id %in% e$no_variance$subj_id);
  
  e$N <- e$disc_df$subj_id%>%unique()%>%length()
...
e$AUCplot <- ggplot(e$AUClong,aes(x=response,y=rcAUC,group=response,fill=response,color=response)) +
        geom_boxplot(size=1,fill='white',outlier.alpha=0)+
        geom_jitter(alpha=0.3,size=2, width=0.2) +
        geom_path(data=e$AUClong%>%group_by(response)%>%summarise(rcAUC=median(rcAUC)),
                  aes(x=response,y=rcAUC,group=1),
  ...
  # ggplot(e$AUClong, aes(x=response,y=rcAUC, color=response)) + 
  # geom_flat_violin() +
  # # geom_line(data=e$AUClong,aes(x=response, y=rcAUC,group=subj_id), color='black',alpha=0.2) +
  # # geom_point(aes(x=response,y=rcAUC, color=response), size=1, fill="white", alpha=0.2, show.legend = FALSE) +  
  # scale_color_manual(values=detection_colors)+
  # scale_fill_manual(values=detection_colors)+
  # theme_classic()+
...
```{r all-distributions, echo=FALSE, fig.cap= "(ref:hists-cap)", out.width = '100%'}
knitr::include_graphics("figures/hists.png")
```

(ref:ROC-cap) Response conditional ROC curves for Experiments 1-6. The area under the curve is a measure of metacognitive sensitivity. Error bars stand for the standard error of the mean. For illustration, the response-conditional ROC curves of the first 20 participants of each Experiment are plotted in low opacity. Below each ROC: distributions of the area under the curve for the two responses, across participants. Same conventions as Fig. \@ref(fig:all-distributions). Stars represent significance in a two-sided t-test: \*: p<0.05, \*\*: p<0.01, \*\*\*: p<0.001

```{r all-rcROCs, echo=FALSE, fig.cap= "(ref:ROC-cap)", out.width = '100%'}
...
Median completion time was `r printnum(this.expt$median_completion_time)` minutes. Mean accuracy was `r printnum(this.expt$disc_df_unfiltered$correct%>%mean())`, and participants reported seeing an *O* on `r printnum(this.expt$disc_df_unfiltered$response%>%as.numeric()%>%mean()-1)` of the trials. In a deviation from our pre-registration, we excluded `r this.expt$no_variance$subj_id%>%length()` participants for having zero variance in their confidence ratings for at least one of the two responses (see Section \@ref(deviations)). Overall we excluded `r this.expt$N_total-this.expt$N` participants, leaving `r this.expt$N` participants for the main analysis. Going forward, only data from included participants is analyzed.
...
```{r RobustnessRegionFunction, echo=FALSE, fig.show='hide', cache=TRUE}
...
# e3$H2$RR<-getRobustnessRegion(e3$AUC$response1-e3$AUC$response2,seq(0.001,2,0.001))
...
Median completion time was `r printnum(this.expt$median_completion_time)` minutes. Mean accuracy was `r printnum(this.expt$disc_df_unfiltered$correct%>%mean())`, and participants reported seeing a vertical line on `r printnum(this.expt$disc_df_unfiltered$response%>%as.numeric()%>%mean()-1)` of the trials. In a deviation from our pre-registration, we excluded `r this.expt$no_variance$subj_id%>%length()` participants for having zero variance in their confidence ratings for at least one of the two responses (see Section \@ref(deviations)). Overall we excluded `r this.expt$N_total-this.expt$N` participants, leaving `r this.expt$N` participants for the main analysis. Going forward, only data from included participants is analyzed. 
...
Median completion time was `r printnum(this.expt$median_completion_time)` minutes. Mean accuracy was `r printnum(this.expt$disc_df_unfiltered$correct%>%mean())`, and participants reported seeing a straight line on `r printnum(this.expt$disc_df_unfiltered$response%>%as.numeric()%>%mean()-1)` of the trials. In a deviation from our pre-registration, we excluded `r this.expt$no_variance$subj_id%>%length()` participants for having zero variance in their confidence ratings for at least one of the two responses (see Section \@ref(deviations)). Overall we excluded `r this.expt$N_total-this.expt$N` participants, leaving `r this.expt$N` participants for the main analysis. Going forward, only data from included participants is analyzed. 
...
Median completion time was `r printnum(this.expt$median_completion_time)` minutes. Mean accuracy was `r printnum(this.expt$disc_df_unfiltered$correct%>%mean())`, and participants reported seeing a downward-tilted cube on `r printnum(this.expt$disc_df_unfiltered$response%>%as.numeric()%>%mean()-1)` of the trials. In a deviation from our pre-registration, we excluded `r this.expt$no_variance$subj_id%>%length()` participants for having zero variance in their confidence ratings for at least one of the two responses (see Section \@ref(deviations)). Overall we excluded `r this.expt$N_total-this.expt$N` participants, leaving `r this.expt$N` participants for the main analysis. Going forward, only data from included participants is analyzed. 
...
*Hypothesis 2*: Contrary to our hypothesis, A Bayes Factor analysis did not provide evidence for or against a difference in auROC for reports of seeing an upward-tilted cube (`r apa_print(t.test(this.expt$AUC$response1))$estimate`) and reports of seeing a downward-tilted cube (`r apa_print(t.test(this.expt$AUC$response2))$estimate`; Cohen's d = `r printnum(this.expt$H2$d)`; `r apa_print(this.expt$H2$BF)$statistic`; see Figure \@ref(fig:all-rcROCs), panel 5.). In contrast, a t-test revealed a significant metacognitive asymmetry, with higher metacognitive sensitivity for perceiving an upward-tilted (default-violating) cube (`r apa_print(this.expt$H2$t.test)$statistic`). See Supplementary Figure \@ref(fig:RR) for a full Robustness Region plot [@dienes2019know].
...
*Hypothesis 4*: Contrary to our hypothesis, response times for 'upward-tilted' responses were similar to response times for 'downward-tilted' responses with a median difference of `r e5$H4$raw_es%>%abs()%>%round()` ms. (`r apa_print(this.expt$H4$t.test)$statistic` ; Cohen's d = `r printnum(this.expt$H4$d)`; `r apa_print(this.expt$H4$BF)$statistic`; see Figure \@ref(fig:all-rcROCs), panel 5).
...
Median completion time was `r printnum(this.expt$median_completion_time)` minutes. Mean accuracy was `r printnum(this.expt$disc_df_unfiltered$correct%>%mean())`, and participants reported seeing a normal *N* on `r printnum(this.expt$disc_df_unfiltered$response%>%as.numeric()%>%mean()-1)` of the trials. In a deviation from our pre-registration, we excluded `r this.expt$no_variance$subj_id%>%length()` participants for having zero variance in their confidence ratings for at least one of the two responses (see Section \@ref(deviations)). Overall we excluded `r this.expt$N_total-this.expt$N` participants, leaving `r this.expt$N` participants for the main analysis. Going forward, only data from included participants is analyzed. 
...
*Hypothesis 2*: In line with our hypothesis, auROC for reports of target presence (`r apa_print(t.test(this.expt$AUC$response1))$estimate`) was higher than for reports of target absence (`r apa_print(t.test(this.expt$AUC$response2))$estimate`; `r apa_print(this.expt$H2$t.test)$statistic`; Cohen's d = `r printnum(this.expt$H2$d)`; `r apa_print(this.expt$H2$BF)$statistic`; see Figure \@ref(fig:rcROC7), left panel). 
...
```{r rcROC7, echo=FALSE, fig.cap= "Response conditional ROC curves (left panel) and confidence and reaction time distributions (right panel) for Exp. 7 (detection positive control). The structure of this figure is similar to Figs 3-4.", out.width = '100%'}
...
formatForConfidenceDatabase <- function(df,stimuli,exp) {
  df %>%
    ...
    mutate(subj_id=factor(subj_id),
           ...
           Stimulus=which_stimulus,
           Response=ifelse(correct==1,
                           which_stimulus,
                           3-as.numeric(which_stimulus)),
           RT_dec=RT/1000,
           Confidence=confidence,
           RT_conf=conf_RT/1000,
           Accuracy=correct,
           Trial=trial)%>%
    select(Subj_idx, Stimulus, Response, RT_dec, Confidence, RT_conf, SOA, Accuracy, Trial)%>%
    arrange(Subj_idx, Trial)%>%
    write.csv(.,file=paste('..\\data\\data_Mazor_2021_Expt',exp,'.csv',sep=''),row.names=FALSE)
}
